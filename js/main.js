...
let mood = "create"
let tmp;
// console.log(title, price, taxes, ads, discount,
...
function getTotal() {
    if (price.value != "") {
        ...
        total.style.background = "#3f497f"
    }
    else {
        total.innerHTML = ""
        total.style.background = "#ddd"
    }
...
function showData() {
    ...
    if (dataPro.length > 0) {
        btnDelete.innerHTML = `<button onclick="deletAll()" id = "deleteAll">Delete All (${dataPro.length})</button>`
    }
    else {
        btnDelete.innerHTML = ""
    }
